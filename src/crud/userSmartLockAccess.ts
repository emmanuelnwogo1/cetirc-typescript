...
router.post('/', async (req, res) => {
  try {
    const user = await UserSmartLockAccess.create(req.body);
...
router.get('/', async (req, res) => {
  try {
    const users = await UserSmartLockAccess.findAll();
    res.json(users);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (updated) {
      const updatedUser = await UserSmartLockAccess.findByPk(req.params.id);
