...
router.get('/', verifyToken, adminMiddleware, async (req, res) => {
    ...
    try {
        ...
        const { rows: users, count: totalUsers } = await PasswordResetRequest.findAndCountAll({
            where: whereClause,
            offset,
            limit: limitNumber,
        });
...
router.get('/:id', verifyToken, adminMiddleware, async (req, res) => {
    try {
        const user = await PasswordResetRequest.findByPk(req.params.id);
        if (!user) {
            res.status(404).json({
                status: 'failed',
...
router.put('/:id', verifyToken, adminMiddleware, async (req, res) => {
    try {
        ...
        if (updated > 0) {
            const updatedUser = await PasswordResetRequest.findByPk(userId);
            res.json({
                status: 'success',
