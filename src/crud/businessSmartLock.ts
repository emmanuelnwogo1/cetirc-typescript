...
router.post('/', async (req, res) => {
  try {
    const user = await BusinessSmartLock.create(req.body);
...
router.get('/', async (req, res) => {
  try {
    const users = await BusinessSmartLock.findAll();
    res.json(users);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
...
router.put('/:id', async (req, res) => {
  try {
    ...
    if (updated) {
      const updatedUser = await BusinessSmartLock.findByPk(req.params.id);
